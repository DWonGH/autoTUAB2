...
class Leaf(nn.Module):
    ...
    def forward(self, x):
        ...
        # print('after complex_conv:',outputs.shape)
        outputs = self._activation(outputs)
        # print('after _activation:',outputs.shape)

        outputs = self._pooling(outputs)
        # print('after _pooling:',outputs.shape)

        outputs = torch.maximum(outputs, torch.tensor(1e-5, device=outputs.device))
        # print('after maximum:',outputs.shape)

        if self._compression:
            outputs = self._compression(outputs)
            # print('after _compression:',outputs.shape)
